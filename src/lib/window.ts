...
/**
 * 某个计划时刻实际会被放行的时间：窗口外则顺延到下一个释放点（开窗点 + 随机延迟）。
 * 窗口内返回原时刻；配置为「不限制」时也返回原时刻。
 *
 * 用于给用户展示「下一次自动同步大约在什么时候」，与调度器 tick 的
 * 放行条件（先看 interval，再看窗口）保持一致。
 */
export function deferIntoWindow(at: number): number {
  if (isWithinWindow(new Date(at))) return at;
  return nextWindowReleaseAt(new Date(at)) ?? at;
}
